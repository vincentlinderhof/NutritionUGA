...
# the plot level now and that farmers
# are asked what percentage of their plot
# was planted with a particular crop.
# -------------------------------------

# read in data and select key variables
crop_prod <- read_dta(file.path(dataPath, "AGSEC4A.dta")) %>%
  select(HHID, parcel_id = parcelID, pltid = plotID,
         crop_code = cropID, harv_area=a4aq7, harv_area2=a4aq9)

# percentage of plot covered in crop
crop_prod$harv_area <- crop_prod$harv_area * crop_prod$harv_area2 / 100
crop_prod$harv_area2 <- NULL
...
land <- read_dta(file.path(dataPath, "AGSEC2A.dta")) %>%  
  select(HHID, parcel_id=parcelID, area_farmer=a2aq5, area_gps=a2aq4)
...
saveRDS(crop_prod_harv_area,   "Data/Crop_prod_harv_area_2011.rds")
saveRDS(crop_prod_area_farmer, "Data/Crop_prod_area_farmer_2011.ds")
saveRDS(crop_prod_area_gps,    "Data/Crop_prod_area_gps_2011.rds")
saveRDS(crop_prod_area_rel,    "Data/Crop_prod_area_rel_2011.rds")
saveRDS(crop_prod_area_mix,    "Data/Crop_prod_area_mix_2011.rds")
